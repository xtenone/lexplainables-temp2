...
Zelfde SQLAlchemy-Core-stijl als `postgres_store.py` op dezelfde engine (`get_engine()`), maar een
...
from .contracts import AgentRun, AnnotatieDocument, AnnotatieElement, AuditRecord
from .models import annotatie_audit, annotatie_documenten
...
class AnnotatieStore:
    async def maak_document(self, doc: AnnotatieDocument) -> None:
        ...
        async with get_engine().begin() as conn:
            await conn.execute(insert(annotatie_documenten).values(
                slug=doc.slug,
                user_id=doc.user_id,
    ...
    async def lijst_documenten(self, user_id: str, limit: int = 50, offset: int = 0) -> list[AnnotatieDocument]:
        async with get_engine().connect() as conn:
            rows = (await conn.execute(
                ...
                .where(annotatie_documenten.c.user_id == user_id)
                .order_by(annotatie_documenten.c.updated.desc())
    ...
    async def verwijder_document(self, slug: str) -> None:
        async with get_engine().begin() as conn:
            await conn.execute(delete(annotatie_audit).where(annotatie_audit.c.document_slug == slug))
            await conn.execute(delete(annotatie_documenten).where(annotatie_documenten.c.slug == slug))
    ...
    async def schrijf_audit(
        ...
        async with get_engine().begin() as conn:
            await conn.execute(insert(annotatie_audit).values(
                document_slug=slug, client_id=client_id, actor=actor, actie=actie,
                element_id=element_id, detail=detail or {}, tijdstip=utcnow(),
            ))
    ...
    async def schrijf_auditregels(self, slug: str, client_id: str, actor: str, regels: list[tuple]) -> None:
        ...
        nu = utcnow()
        async with get_engine().begin() as conn:
            await conn.execute(insert(annotatie_audit), [
                {
    ...
    async def lees_audit(self, slug: str, limit: int = 200, offset: int = 0) -> list[AuditRecord]:
        ...
        return [
            AuditRecord(
                ...
                tijdstip=aware(r._mapping["tijdstip"]),
            )
            for r in rows
        ]

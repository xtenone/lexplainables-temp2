...
async def test_create_slaat_alleen_hash_op(db):
    from app.features.api_tokens import store as api_tokens
    ...
    from app.features.api_tokens.models import api_tokens as api_tokens_tabel

    async with db.get_engine().connect() as conn:
        row = (await conn.execute(select(api_tokens_tabel))).mappings().first()

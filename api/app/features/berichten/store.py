...
Beheerders schrijven berichten (draft → gepubliceerd); analisten lezen ze via het panel in de
navigatie. Leesbewijzen (`bericht_leesbewijzen`) registreren per user welke berichten al gezien
zijn.
"""
...
from .models import bericht_leesbewijzen, berichten
...
def _row_to_dict(row, *, gelezen: bool | None = None) -> dict:
    ...
    result = {
        ...
        "gepubliceerd_op": aware(gp_op) if gp_op else None,
...
def _user_created_subq(userid: str):
    return select(users.c.created).where(users.c.userid == userid).scalar_subquery()
...
async def maak_bericht(
    ...
    async with get_engine().begin() as conn:
        result = await conn.execute(insert(berichten).values(**values).returning(*berichten.c))
...
async def verwijder_bericht(bericht_id: int) -> None:
    ...
    async with get_engine().begin() as conn:
        bestaat = await conn.scalar(
            select(berichten.c.id).where(berichten.c.id == bericht_id)
        )
        if bestaat is None:
            raise BerichtError(f"Bericht {bericht_id} niet gevonden.")
        await conn.execute(
        ...
        await conn.execute(
            delete(berichten).where(berichten.c.id == bericht_id)
        )

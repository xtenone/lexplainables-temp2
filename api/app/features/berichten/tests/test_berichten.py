...
async def _insert_user(db, userid: str, email: str | None = None) -> None:
    ...
    from app.features.identiteit_toegang.models import users
...
async def test_maak_en_lijst(db):
    from app.features.berichten import store as svc
...
async def test_verwijder_cascade_leesbewijzen(db):
    ...
    async with _db.get_engine().connect() as conn:
        cnt = await conn.scalar(
            select(func.count()).select_from(bericht_leesbewijzen)
            .where(bericht_leesbewijzen.c.bericht_id == row["id"])
        )
    assert cnt == 1
...
async def test_markeer_alles_gelezen_concurrent(db):
    ...
    async with _db.get_engine().connect() as conn:
        cnt = await conn.scalar(
            select(func.count()).select_from(bericht_leesbewijzen)
            .where(bericht_leesbewijzen.c.bericht_id == row["id"])
            .where(bericht_leesbewijzen.c.userid == "concurrent-user")
        )
    assert cnt == 1

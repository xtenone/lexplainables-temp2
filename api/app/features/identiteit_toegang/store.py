"""Store-abstractie voor het identiteit_toegang-domein (werkwijze-ADR-0007).
...
Verantwoordelijkheden:
  ...
  - Admin-CRUD voor gebruikersbeheer, met de beleidsgaranties (laatste actieve beheerder mag niet
    verdwijnen).
  - Optionele 2FA (TOTP): koppelen (`begin_2fa`), bevestigen (`activate_2fa`) en losmaken
    (`disable_2fa`). Het secret staat versleuteld (Fernet, zie shared.secrets_crypto).
...
from ...shared.secrets_crypto import crypto_beschikbaar, decrypt, decrypt_ttl, encrypt
from .models import ROLLEN, User, users
...
def _row_to_user(row) -> User:
    ...
    return User(
        ...
        active=m["active"],
        sessions_valid_from=aware(m["sessions_valid_from"]),
        created=aware(m["created"]),
        updated=aware(m["updated"]),
    )
...
async def get_user(userid: str) -> User | None:
    async with get_engine().connect() as conn:
        row = (await conn.execute(
            select(users).where(users.c.userid == _norm_userid(userid))
        )).mappings().first()
...
async def get_user_by_email(email: str) -> User | None:
    async with get_engine().connect() as conn:
        row = (await conn.execute(
            select(users).where(users.c.email == _norm_email(email))
        )).mappings().first()
    return _row_to_user(row) if row is not None else None
...
async def list_users() -> list[User]:
    async with get_engine().connect() as conn:
        rows = (await conn.execute(select(users).order_by(users.c.userid))).mappings().all()
    return [_row_to_user(r) for r in rows]


async def _count() -> int:
    async with get_engine().connect() as conn:
        return (await conn.execute(select(func.count()).select_from(users))).scalar() or 0
    ...
    async with get_engine().connect() as conn:
        return (await conn.execute(
            select(func.count()).select_from(users).where(
                users.c.role == "beheerder", users.c.active.is_(True)
            )
        )).scalar() or 0
...
async def patch_user(userid: str, *, role: str | None = None, active: bool | None = None) -> User:
    ...
    waarden["updated"] = utcnow()
    async with get_engine().begin() as conn:
        if was_actieve_beheerder and not wordt_actieve_beheerder:
            if get_engine().url.get_backend_name() == "postgresql":
                await conn.execute(
        ...
        await conn.execute(update(users).where(users.c.userid == userid).values(**waarden))
...
async def reset_password(userid: str) -> tuple[User, str]:
    ...
    user = await _require_user(userid)
    tijdelijk = genereer_wachtwoord()
    await _update(user.userid, password_hash=hash_password(tijdelijk), sessions_valid_from=utcnow())
    return user, tijdelijk
...
async def change_own_password(userid: str, current: str, nieuw: str) -> None:
    ...
    await _update(user.userid, password_hash=hash_password(nieuw), sessions_valid_from=utcnow())
...
async def delete_user(userid: str) -> None:
    ...
    async with get_engine().begin() as conn:
        await conn.execute(delete(users).where(users.c.userid == user.userid))

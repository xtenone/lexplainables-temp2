...
Verantwoordelijkheden:
  ...
  - `resolve_config`: zet een profielnaam om in een `LlmConfig` (ontsleutelt de API-key en valt
    voor lege velden terug op de env-config), die de LLM-adapter gebruikt.
...
from ...shared.secrets_crypto import decrypt, encrypt
from .llm.base import LlmConfig
from .models import LlmProfile, llm_profiles
...
async def list_profiles() -> list[LlmProfile]:
    async with get_engine().connect() as conn:
        rows = (await conn.execute(
            select(llm_profiles).order_by(llm_profiles.c.name)
        )).mappings().all()
...
async def get_profile(name: str) -> LlmProfile | None:
    async with get_engine().connect() as conn:
        row = (await conn.execute(
            select(llm_profiles).where(llm_profiles.c.name == name)
        )).mappings().first()
...
async def get_default() -> LlmProfile | None:
    async with get_engine().connect() as conn:
        row = (await conn.execute(
            select(llm_profiles).where(llm_profiles.c.is_default.is_(True))
        )).mappings().first()
    return _row_to_profile(row) if row is not None else None
...
async def _count() -> int:
    async with get_engine().connect() as conn:
        return (await conn.execute(select(func.count()).select_from(llm_profiles))).scalar() or 0
    ...
    *,
    ...
    async with get_engine().begin() as conn:
        if profile.is_default:
            await conn.execute(
                update(llm_profiles).where(llm_profiles.c.is_default.is_(True))
        ...
        if is_new:
            await conn.execute(insert(llm_profiles).values(
                name=profile.name, created=profile.created, **waarden
            ))
        else:
            await conn.execute(
                update(llm_profiles).where(llm_profiles.c.name == name).values(**waarden)
            )
    return profile
...
async def set_default(name: str) -> LlmProfile:
    ...
    async with get_engine().begin() as conn:
        ...
        await conn.execute(
            update(llm_profiles).where(llm_profiles.c.name == name)
            .values(is_default=True, updated=utcnow())
        )
    profile.is_default = True
    return profile
...
async def delete_profile(name: str) -> None:
    ...
    async with get_engine().begin() as conn:
        await conn.execute(delete(llm_profiles).where(llm_profiles.c.name == name))

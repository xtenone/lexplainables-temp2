...
def _fresh_settings(monkeypatch, **env):
    ...
    from app.shared import secrets_crypto
...
async def test_resolve_config_valt_terug_op_env(monkeypatch, db):
    ...
    from app.features.llm_profielen import store as profiles
...
async def test_test_route_lekt_geen_requestdetails(admin_client, monkeypatch):
    ...
    monkeypatch.setattr("app.features.llm_profielen.router.build_llm_client", lambda cfg: BoomClient())
...
async def test_test_route_rate_limited(admin_client, monkeypatch):
    ...
    monkeypatch.setattr("app.features.llm_profielen.router.build_llm_client", lambda cfg: OkClient())
    codes = [

...
async def require_admin(credentials: HTTPAuthorizationCredentials | None = Depends(_bearer)) -> str:
    ...
    if presented:
        ...
        if env_id is not None:
            return env_id
        # Lazy import om een import-cyclus bij het laden te vermijden (api_tokens → db); shared/
        # importeert hier bewust uit features/ (net als lexplainables' shared/auth.py) — dit is de
        # enige plek waar shared/ iets uit features/ nodig heeft.
        from ..features.api_tokens import store as api_tokens
